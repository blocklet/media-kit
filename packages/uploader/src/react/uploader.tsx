								...
								createRoot(root).render(
									<AIImageShowPanel
										...
										onSelect={(data: any) => {
											...
											data.forEach(
												({ src: base64, alt }: any, index: number) => {
...
export function initUploader(props: any) {
	...
	let currentUppy = new Uppy({
		...
		async onBeforeRequest(req, file) {
			...
			const componentDid =
				window?.uploaderComponentId || window?.blocklet?.componentId;
			if (componentDid) {
				// @ts-ignore
		...
		onAfterResponse: async (req, res) => {
			...
			if (
				["PATCH", "POST"].includes(result.method) &&
				[200, 500].includes(result.status)
			) {
				const isExist = [true, "true"].includes(
					result.headers["x-uploader-file-exist"],
				);
				const uploadURL = getUrl(
					result.url,
					result.headers["x-uploader-file-name"],
				); // upload URL with file name
			...
			const shouldAutoCloseAfterDropUpload = currentUppy
				.getFiles()
				.every((item: any) => item.source === "DropTarget");
	...
	const appendUploadIdEvent = ({
		fileIDs,
		id,
	}: { fileIDs: string[]; id: string }) => {
...
export const Uploader = forwardRef((props: UploaderProps, ref: any) => {
	...
	const plugins = uniq([
		..._plugins,
		...pluginList.filter((item) => item.alwayUse).map((item) => item.id),
	]);

	const { loading: loadingStatus } = useRequest(
		async () => {
			let restrictions = (
				!isNil(props?.coreProps?.restrictions)
					? cloneDeep(props?.coreProps?.restrictions)
					: {}
			) as any;
			...
			if (
				!restrictions?.allowedFileTypes?.length &&
			...
			if (
				...
				typeof restrictions.maxFileSize === "string"
			) {
				restrictions.maxFileSize =
					xbytes.parseSize(restrictions.maxFileSize, { iec: false }) ||
					undefined;
			}
	...
	const withoutAnyAllowedFileTypes =
		typeof state.restrictions?.allowedFileTypes === "object" &&
		state.restrictions?.allowedFileTypes?.length === 0;
	...
	if (withoutAnyAllowedFileTypes) {
		note = get(
			localeMap,
			`${locale}.strings.noAllowedFileTypes`,
		...
		note =
			get(
				localeMap,
				`${locale}.strings.allowedFileTypes`,
				"Allowed file types: ",
			) + extsNote;
	}
	...
	useEffect(() => {
		...
		pluginHandlers.forEach(({ name, event }) => {
			if (plugins.includes(name)) {
				...
				state.uppy.on(event, (files: Object[]) =>
					handlePluginSelection(files, state, props, name),
				);
			}
		});
	}, [
	...
	const handleFocus = useCallback((event: MouseEvent | TouchEvent) => {
		if (
			isDebug &&
			uploaderContainerRef.current &&
			!uploaderContainerRef.current.contains(event.target as Node)
		) {
				...
				<Box
					...
					sx={{
						...
						".uppy-Dashboard-Item-previewInnerWrap, .uppy-ProviderBrowserItem-inner":
							{
								background:
									"repeating-conic-gradient(#bdbdbd33 0 25%,#fff 0 50%) 50%/16px 16px !important",
							},
						".uploaded-add-item": {
					...
					{state.uppy && (
						<Dashboard
							...
							fileManagerSelectionType={
								state.restrictions?.maxNumberOfFiles === 1 ? "files" : "both"
							}
